...
The 'Show as-of Date' setting controls whether or not the last update time is to be displayed in the top right of the panel.  Most users can ignore this setting. When it is set, the maximum datetime value in the query record set is displayed alongside the panel title, which can be useful in process monitoring applications.  The format field controls how this time is displayed (see documentation for [moment.js](https://momentjs.com/guides/#/parsing/known-formats/) for formatting details), or use the reserved keyword 'ELAPSED' to display as a natural language string, relative to the current time.   Help is available, if needed.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQzMzQyNDA1OSwzOTgxMjMyNTcsLTEwND
