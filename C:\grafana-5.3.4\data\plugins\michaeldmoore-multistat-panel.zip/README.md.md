...
In the event that the database cannot pre-filter the data in this way, the aggregation setting can still generate the required display, but at the cost of increased CPU and network load.  Generally, this is not significant - Multistat can easily handle queries with a few hundred labels, each with a hundred or more rows.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzYzNjY5OTcwLDE4MTI0Nzk4NzMsLTI2ND
