			...
			_export('PanelCtrl', MultistatPanelCtrl = function (_MetricsPanelCtrl) {
         	...
         	.attr("stroke", "white");*/
									...
									if (panel.ShowLabels) {
										...
										gg.append("text").text(function (d) {
											return d[labelCol];
										}).attr("x", left).attr("y", function (d, i) {
											return labelScale(d[labelCol]) + labelScale.bandwidth() / 2;
										...
										}).attr("text-anchor", "start").attr("dominant-baseline", "central").on("mouseover", function (d) {
									...
									if (panel.ShowValues && panel.ValuePosition != "top") {
										g.append("text").text(function (d) {
											...
											if (panel.ValuePosition == "bar base") return valueScale(baseLineValue) + 5;else {
												var val = scaleAndClipValue(d[valueCol]);
											...
											if (panel.ValuePosition == "bar base") return "start";else return d[valueCol] * ScaleFactor > baseLineValue ? "end" : "start";
									...
									if (highSideMargin > 0) {
										var gg = svg.append("g").attr("transform", 'translate(1,' + (hh + highSideMargin) + ')').attr("class", "michaeldmoore-multistat-panel-valueaxis").call(d3.axisTop(valueScale));
									...
									if (lowSideMargin > 0) {
										var gg = svg.append("g").attr("transform", 'translate(0,' + (hh + dh - lowSideMargin) + ')').attr("class", "michaeldmoore-multistat-panel-valueaxis").call(d3.axisBottom(valueScale));
								...
								if (groupCol >= 0) {
									...
									if (GroupNameFilter.length > 0) {
										...
										for (var i = 0; i < this.groupedRows.length; i++) {
											...
											if (groupName.match(regex) != null) matchingGroups.push(this.groupedRows[i]);
										}
								...
								var oddeven = false;
								//var svg = 
								this.svg.selectAll("rect").data(this.rows).enter().append("rect").attr("class", "michaeldmoore-multistat-panel-row").attr("width", labelScale.bandwidth()).attr("height", h).attr("x", function (d, i) {
									return labelScale(d[labelCol]);
								}).attr("y", chartTop).attr("fill", function (d) {
									oddeven = !oddeven;
									return oddeven ? OddRowColor : EvenRowColor;
								}).on("mouseover", function (d) {
									if (showTooltips) tooltipShow(d, cols, id);
								...
								var gg = this.svg.selectAll("text").data(this.rows).enter().append("g");
								...
								if (this.panel.ShowValues && this.panel.ValuePosition == "top") {
									...
									gg.append("text").text(function (d) {
										...
										return labelScale(d[labelCol]) + labelScale.bandwidth() / 2;
									}).attr("y", 0).attr("font-family", "sans-serif").attr("font-size", this.panel.ValueFontSize).attr("fill", this.panel.ValueColor).attr("text-anchor", "middle").attr("dominant-baseline", "text-before-edge").each(function (d, i) {
								...
								if (this.panel.ShowLabels) {
									...
									.attr("font-family", "sans-serif").attr("font-size", this.panel.LabelFontSize).attr("fill", function (d, i) {
										return d[valueCol] * ScaleFactor > maxLineValue || d[valueCol] * ScaleFactor < minLineValue ? OutOfRangeLabelColor : LabelColor;
									}).attr("text-anchor", "middle").attr("dominant-baseline", "text-after-edge").on("mouseover", function (d) {
										if (showTooltips) tooltipShow(d, cols, id);
									}).on("mouseout", function () {
										tooltipHide(id);
									}).each(function (d, i) {
								...
								var valueScale = d3.scaleLinear().domain([maxLineValue, minLineValue]).range([chartTop, h]).nice();
								...
								if (this.panel.ShowValues && this.panel.ValuePosition != "top") {
									gg.append("text").text(function (d) {
										...
										if (ValuePosition == "bar base") return valueScale(baseLineValue) + 5;else {
											var val = scaleAndClipValue(d[valueCol]);
											return valueScale(val) + (val > baseLineValue ? -5 : +5);
										}
										...
										if (ValuePosition == "bar base") return "text-after-edge";else return d[valueCol] * ScaleFactor > baseLineValue ? "text-before-edge" : "text-after-edge";
									});
								}
